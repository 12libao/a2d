...
int main(int argc, char *argv[]) {
  ...
  // print vol and dvdx based on type of T
  std::string printfCmd = "volume: ";
  if (typeid(double) == typeid(vol)) {
    printfCmd += "%f\n";
  } else if (typeid(std::complex<double>) == typeid(vol)) {
    printfCmd += "%f + %fi\n";
  } else {
    printfCmd += "Invalid type\n";
  }

  std::printf(printfCmd.c_str(), std::real(vol),
              std::imag(vol));  // Print the value

  // print the norm of dvdx
  printfCmd = "norm of dvdx: ";
  if (typeid(double) == typeid(vol)) {
    printfCmd += "%f\n";
  } else if (typeid(std::complex<double>) == typeid(vol)) {
    printfCmd += "%f + %fi\n";
  ...
  std::printf(printfCmd.c_str(), std::real(A2D::BLAS::norm(*dvdx)),
              std::imag(A2D::BLAS::norm(*dvdx)));  // Print the value
