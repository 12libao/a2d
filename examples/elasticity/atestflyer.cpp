...
// EXERCISE hint: KokkosBlas1_dot.hpp, KokkosBlas2_gemv.hpp,
// KokkosBlas1_team_dot.hpp
...
#define MemSpace Kokkos::CudaSpace
    ...
    for (int repeat = 0; repeat < nrepeat; repeat++) {
      ...
      // EXERCISE: Convert from hierarchical parallel execution to using
      // KokkosKernels BLAS functions EXERCISE hint: KokkosBlas::gemv (tmp =
      // A*x)
    ...
    for (int repeat = 0; repeat < nrepeat; repeat++) {
      ...
      Kokkos::parallel_reduce(
          ...
          KOKKOS_LAMBDA(const member_type &teamMember, double &update) {
            ...
            // EXERCISE: Multiply each row of matrix A2 with vector x2 using
            // team-based dot functions EXERCISE hint: -
            // KokkosBlas::Experimental::dot (temp2 = <A2(row,:),x2>)
            //                - team-based dot, take subviews of View A2
            double temp2 = KokkosBlas::Experimental::dot(
                teamMember, Kokkos::subview(A2, row, Kokkos::ALL()), x2);
    ...
    printf(
        "    N( %d ) M( %d ) nrepeat ( %d ) problem( %g MB ) time( %g s ) "
        "bandwidth( %g GB/s )\n",
        N, M, nrepeat, Gbytes * 1000, time, Gbytes * nrepeat / time2);
  }
